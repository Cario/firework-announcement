...
/** Grass, plan 3.2: ~520 blades over the entire band, depth 0 at the horizon. */
const BLADE_COUNT_REF = 520;
const BLADE_MIN_H = 4;
const BLADE_DEPTH_H = 24;
const BLADE_MIN_W = 1.2;
const BLADE_DEPTH_W = 1.6;
const BLADE_LEAN_DEG = 14;
...
export function createGround(metrics, options = {}) {
  ...
  function buildBlades(rng) {
    // 520 at the reference width, scaled by width, with a floor of 430 so a
    // phone's shorter, narrower band still reads as turf rather than as a few
    // spikes on a green field.
    const count = clamp(Math.round(BLADE_COUNT_REF * m.density), 430, 900);
    ...
    for (let i = 0; i < count; i++) {
      ...
      const d = rng.next();
      const h = (BLADE_MIN_H + d * BLADE_DEPTH_H) * m.scale;
      const w = (BLADE_MIN_W + d * BLADE_DEPTH_W) * m.scale;
    ...
    // Far blades first: correct occlusion, and it groups the two colours into
    // two contiguous runs so `fillStyle` is only assigned twice per frame.
  ...
  return {
    ...
    draw(ctx, camera) {
      ...
      for (let i = 0; i < trees.length; i++) {
        ...
        ctx.beginPath();
        ctx.moveTo(t.x, m.baseLineY - t.h);
      ...
      for (let i = 0; i < blades.length; i++) {
        const b = blades[i];
        const next = b.d > 0.5 ? PALETTE.blade : PALETTE.bladeLo;
        if (next !== colour) {
          colour = next;
          ctx.fillStyle = colour;
        }
